        ...
        <div className="fixed inset-0 transition-opacity" aria-hidden="true" onClick={onClose}>
          ...
          <form onSubmit={handleSubmit}>
            <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
              <h3 className="text-lg leading-6 font-medium text-kotoba-text">Link a New Video</h3>
              <div className="mt-4 space-y-4">
                <div>
                  <label htmlFor="title" className="block text-sm font-medium text-kotoba-text/80">Video Title</label>
                  <input type="text" id="title" value={title} onChange={(e) => setTitle(e.target.value)} required className="mt-1 block w-full shadow-sm sm:text-sm border-kotoba-text/20 rounded-md py-2 px-3" />
                </div>
                <div>
                  <label htmlFor="url" className="block text-sm font-medium text-kotoba-text/80">Video URL</label>
                  <input type="url" id="url" value={url} onChange={(e) => setUrl(e.target.value)} required className="mt-1 block w-full shadow-sm sm:text-sm border-kotoba-text/20 rounded-md py-2 px-3" />
                </div>
                <h4 className="text-md font-medium text-kotoba-text/90 pt-2">Supplementary Resources (Optional)</h4>
                {resources.map((resource, index) => (
                  <div key={index} className="flex items-center space-x-2">
                    <input type="text" placeholder="Resource Title" value={resource.title} onChange={(e) => handleResourceChange(index, 'title', e.target.value)} className="flex-1 block w-full shadow-sm sm:text-sm border-kotoba-text/20 rounded-md py-2 px-3" />
                    <input type="url" placeholder="Resource URL" value={resource.url} onChange={(e) => handleResourceChange(index, 'url', e.target.value)} className="flex-1 block w-full shadow-sm sm:text-sm border-kotoba-text/20 rounded-md py-2 px-3" />
