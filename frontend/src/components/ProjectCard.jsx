...
const ProjectCard = ({ project, currentUser, onFollow, onUnfollow }) => {
  ...
  return (
    <div className="bg-white border border-kotoba-text/10 rounded-lg shadow-sm overflow-hidden flex flex-col h-full">
      <Link to={`/projects/${project.id}`}>
        <div className="h-48 bg-kotoba-text/10 flex items-center justify-center overflow-hidden">
          <img src={imageUrl} alt={project.title} className="w-full h-full object-cover" />
        </div>
      </Link>
      <div className="p-6 flex flex-col flex-grow">
        <h3 className="text-lg font-semibold text-kotoba-text mb-2">
          <Link to={`/projects/${project.id}`} className="hover:text-kotoba-primary">{project.title}</Link>
        </h3>
        <p className="text-sm text-kotoba-text/70 mb-4 flex-grow">{project.description.substring(0, 100)}...</p>
        ...
        <div className="mb-4">
          ...
          <div className="w-full bg-kotoba-text/10 rounded-full h-2.5">
            <div 
              className="bg-kotoba-primary h-2.5 rounded-full" 
              style={{ width: `${percentage > 100 ? 100 : percentage}%` }}
            ></div>
          </div>
          <div className="text-right text-sm font-medium text-kotoba-text/80 mt-1">
            {Math.round(percentage)}% funded
          </div>
        </div>

        <div className="mt-auto pt-4 border-t border-kotoba-text/10">
          <div className="flex items-center">
            <Link to={`/profile/${project.teacher_id}`} className="mr-3">
              <Avatar
                src={project.teacher_avatar_url}
            ...
            <div className="flex-grow">
              <div className="flex items-center">
                <Link to={`/profile/${project.teacher_id}`} className="text-sm font-medium text-kotoba-text hover:text-kotoba-primary">{project.teacher_name}</Link>
                <VerifiedBadge languages={project.teacher_verified_languages} />
              </div>
              <p className="text-sm text-kotoba-text/60">{project.language} - {project.level}</p>
            </div>
            {canFollow && (
              isFollowing ? (
                <button onClick={handleUnfollowClick} className="bg-kotoba-text/10 text-kotoba-text/80 px-3 py-1 rounded-md text-sm font-medium">Unfollow</button>
              ) : (
