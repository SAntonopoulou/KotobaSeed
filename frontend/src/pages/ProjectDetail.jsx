...
const ProjectDetail = () => {
  ...
  // Auth state from shared SSO cookie (per-origin localStorage would
  // misreport this as "signed out" on any subdomain the user reached
  // via the cookie rather than a direct login form submission).
  // `token` is read by useEffect deps + the pledge gate below. Without
  // it the page throws ReferenceError on mount and goes white.
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="lg:grid lg:grid-cols-3 lg:gap-8">
        <div className="lg:col-span-2">
          ...
          {isOwner && isEditingImage && (
            <div className="mb-6 p-4 border rounded-lg bg-kotoba-background/40 shadow-sm">
              <label htmlFor="project_image_url" className="block text-sm font-medium text-kotoba-text/80">Project Image URL</label>
              <input type="url" id="project_image_url" className="mt-1 block w-full border border-kotoba-text/20 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-kotoba-primary focus:border-kotoba-primary sm:text-sm" value={newImageUrl} onChange={(e) => setNewImageUrl(e.target.value)} placeholder="https://example.com/your-image.png" />
              <div className="mt-3 flex justify-end space-x-3">
                <button onClick={() => setIsEditingImage(false)} className="bg-white py-2 px-4 border border-kotoba-text/20 rounded-md shadow-sm text-sm font-medium text-kotoba-text/80 hover:bg-kotoba-background/40">Cancel</button>
          ...
          <h1 className="text-3xl font-extrabold text-kotoba-text sm:text-4xl mb-4">{project.title}</h1>
          <div className="flex flex-wrap items-center gap-2 mb-6">
            ...
            {tags.map((tag, index) => <span key={index} className="inline-flex items-center px-3 py-0.5 rounded-full text-sm font-medium bg-kotoba-background/60 text-kotoba-text/90">{tag}</span>)}
          </div>
          <div className="mb-6 text-kotoba-text/60 text-sm flex items-center gap-4">
            ...
            {canFollow && (
              isFollowing ? (
                <button onClick={() => handleUnfollow(project.teacher_id)} className="bg-kotoba-text/10 text-kotoba-text/80 px-3 py-1 rounded-md text-sm font-medium">Unfollow</button>
              ) : (
          ...
          <div className="prose prose-indigo max-w-none text-kotoba-text/60 mb-8"><p className="whitespace-pre-line">{project.description}</p></div>

          {project.is_series && (
            <div className="mt-8 border-t border-kotoba-text/10 pt-8">
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-2xl font-bold text-kotoba-text">Series Introduction</h2>
                {isOwner && (
              ...
              {isEditingIntroVideo ? (
                <div className="p-4 border rounded-lg bg-kotoba-background/40 shadow-sm">
                  <label htmlFor="series_intro_video_url" className="block text-sm font-medium text-kotoba-text/80">Intro Video URL</label>
                  <input type="url" id="series_intro_video_url" className="mt-1 block w-full border border-kotoba-text/20 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-kotoba-primary focus:border-kotoba-primary sm:text-sm" value={newIntroVideoUrl} onChange={(e) => setNewIntroVideoUrl(e.target.value)} placeholder="https://youtube.com/watch?v=..." />
                  <div className="mt-3 flex justify-end space-x-3">
                    <button onClick={() => setIsEditingIntroVideo(false)} className="bg-white py-2 px-4 border border-kotoba-text/20 rounded-md shadow-sm text-sm font-medium text-kotoba-text/80 hover:bg-kotoba-background/40">Cancel</button>
          ...
          <div className="mt-8 border-t border-kotoba-text/10 pt-8">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-2xl font-bold text-kotoba-text">Project Videos</h2>
            ...
            {videos.length > 0 ? (
              <div className="space-y-8">
                {videos.map(video => (
                  <div key={video.id} className="bg-white p-4 rounded-lg shadow border border-kotoba-text/10">
                    <h3 className="text-lg font-medium text-kotoba-text mb-4">{video.title}</h3>
                    <VideoPlayer url={video.url} />
                    {video.resources && video.resources.length > 0 && (
                      <div className="mt-4 pt-4 border-t">
                        <h4 className="text-sm font-medium text-kotoba-text/90 mb-2">Resources:</h4>
            ...
            ) : <p className="text-kotoba-text/60">No videos have been added yet.</p>}
          </div>
          ...
          {canRate && (
            <div ref={ratingSectionRef} className="mt-8 border-t border-kotoba-text/10 pt-8">
              <h2 className="text-2xl font-bold text-kotoba-text mb-4">Your Rating</h2>
          ...
          {reviews.length > 0 && (
            <div className="mt-8 border-t border-kotoba-text/10 pt-8">
              <h2 className="text-2xl font-bold text-kotoba-text mb-4">Reviews</h2>
              <div className="space-y-6">
                {reviews.map((review) => (
                  <div key={review.id} className="bg-white p-4 rounded-lg shadow-sm border border-kotoba-text/5">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-sm text-kotoba-text/60">{formatDateShort(review.created_at)}</span>
                    </div>
                    {review.comment && <p className="mt-3 text-kotoba-text/80 italic">"{review.comment}"</p>}
                    {review.teacher_response && (<div className="mt-4 pt-4 border-t border-kotoba-text/10"><p className="text-sm font-semibold text-kotoba-text/90">Response from {project.teacher_name}:</p><p className="text-sm text-kotoba-text/70 italic">"{review.teacher_response}"</p><p className="text-xs text-kotoba-text/40 text-right">{formatDateShort(review.response_created_at)}</p></div>)}
          ...
          <div className="mt-8 border-t border-kotoba-text/10 pt-8">
            <h2 className="text-2xl font-bold text-kotoba-text mb-4">Project Updates</h2>
            {isOwner && (
              <div className="mb-6"><textarea className="w-full border border-kotoba-text/20 rounded-md p-2 text-sm" rows={3} placeholder="Post an update for your backers..." value={newUpdate} onChange={(e) => setNewUpdate(e.target.value)} /><div className="mt-2 text-right"><button onClick={handlePostUpdate} className="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-white bg-kotoba-primary hover:bg-kotoba-primary/90 focus:outline-none">Post Update</button></div></div>
            )}
            {updates.length === 0 ? (<p className="text-kotoba-text/60 text-sm">No updates yet.</p>) : (
              <div className="space-y-4">
                {updates.map(update => (
                  <div key={update.id} className="bg-kotoba-background/40 p-4 rounded-lg border border-kotoba-text/10">
                    {editingUpdateId === update.id ? (
                      <div><textarea className="w-full border border-kotoba-text/20 rounded-md p-2 text-sm" rows={3} value={editingContent} onChange={(e) => setEditingContent(e.target.value)} /><div className="mt-2 text-right space-x-2"><button onClick={handleCancelEdit} className="text-sm text-kotoba-text/70">Cancel</button><button onClick={() => handleSaveUpdate(update.id)} className="text-sm text-kotoba-primary font-medium">Save</button></div></div>
                    ) : (
                      <><p className="text-kotoba-text/90 whitespace-pre-line">{update.content}</p><div className="flex justify-between items-center mt-2"><p className="text-xs text-kotoba-text/60">{formatDateShort(update.created_at)}</p>{isOwner && (<button onClick={() => handleEditUpdate(update)} className="text-xs text-kotoba-primary hover:underline">Edit</button>)}</div></>
                    )}
                  </div>
                ))}
              </div>
        ...
        <div className="mt-8 lg:mt-0">
          <div className="bg-white p-6 rounded-lg shadow-lg border border-kotoba-text/10 sticky top-6">
            <div className="mb-6">
              <div className="flex justify-between text-base font-medium text-kotoba-text mb-1"><span>{formatCurrency(project.current_funding)}</span><span className="text-kotoba-text/60">goal {formatCurrency(project.funding_goal)}</span></div>
              <div className="w-full bg-kotoba-text/10 rounded-full h-3"><div className="bg-kotoba-primary h-3 rounded-full transition-all duration-500" style={{ width: `${percentage}%` }}></div></div>
              <p className="mt-2 text-sm text-kotoba-text/60 text-right">{Math.round(percentage)}% funded</p>
            </div>
            {project.is_series && project.num_videos && (
              <p className="text-sm text-kotoba-text/60 mb-3">Expected videos: {project.num_videos}</p>
            )}
            {project.status === 'funding' ? (token ? (<PledgeForm projectId={project.id} projectName={project.title} />) : (<div className="text-center"><p className="text-kotoba-text/70 mb-4">Log in to back this project.</p><Link to="/login" className="block w-full bg-kotoba-primary text-white text-center py-2 px-4 rounded-md hover:bg-kotoba-primary/90">Login to Pledge</Link></div>)) : (<div className="bg-kotoba-background/60 p-4 rounded text-center text-kotoba-text/70">This project is {project.status.replace(/_/g, ' ')}.</div>)}
            ...
            {['successful', 'completed', 'pending_confirmation'].includes(project.status) && project.teacher_stripe_account_id && (
              <div className="mt-6 pt-6 border-t border-kotoba-text/10">
                {project.total_tipped_amount > 0 && (
                  <p className="text-sm text-kotoba-text/60 mb-3">Total tips received: {formatCurrency(project.total_tipped_amount)}</p>
                )}
                {!isOwner && <TipForm projectId={project.id} />}
              </div>
            )}

            {backers.length > 0 && (<div className="mt-6 pt-6 border-t border-kotoba-text/10"><h4 className="text-sm font-medium text-kotoba-text mb-3">Backers ({backers.length})</h4><div className="flex flex-wrap gap-2">{backers.map(backer => (<Link key={backer.id} to={`/profile/${backer.id}`} title={backer.full_name}><Avatar src={backer.avatar_url} name={backer.full_name} size={40} /></Link>))}</div></div>)}
            {project.deadline && (<div className="mt-6 pt-6 border-t border-kotoba-text/10"><p className="text-sm text-kotoba-text/60">Deadline: {formatDateShort(project.deadline)}</p></div>)}
            {deliveryText && (<div className="mt-6 pt-6 border-t border-kotoba-text/10"><p className="text-sm text-kotoba-text/60">{deliveryText}</p></div>)}
          </div>
        </div>
      </div>

      {relatedProjects.length > 0 && (<div className="mt-12 border-t border-kotoba-text/10 pt-12"><h2 className="text-2xl font-bold text-kotoba-text mb-6">More like this</h2><div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">{relatedProjects.map(p => (<ProjectCard key={p.id} project={p} currentUser={currentUser} onFollow={handleFollow} />))}</div></div>)}
