...
// SophiaBookingDialog — themed equivalent of components/BookingDialog,
// 1:1 logic port of DafniBookingDialog (slot fetch, cutoff guard,
// recurring opt-in, intro message, trial branch). Only visuals change:
// bone dialog body, navy hairline border, Playfair italic title,
// navy selected-slot pills. Wrapped in `.theme-sophia-inkwell-site`
...
const SophiaBookingDialog = ({ pack, tutorDisplayName, onClose }) => {
  ...
  // ESC closes the dialog — matches the rest of the Inkwell theme drawer.
  useEffect(() => {
  ...
  return (
    <div
      className="theme-sophia-inkwell-site s-bd-overlay"
      onClick={onClose}
    >
      <div
        onClick={(e) => e.stopPropagation()}
        className="s-bd-dialog"
        role="dialog"
        aria-modal="true"
        aria-label={pack.name}
      >
        <div className="s-bd-head">
          <div>
            <h2 className="s-bd-title">{pack.name}</h2>
            <p className="s-bd-sub">
        ...
        <div>
                    ...
                    {daySlots.map((slot) => {
                      ...
                      return (
                        <button
                          ...
                          className={`s-bd-slot${isPicked ? ' s-bd-slot-picked' : ''}`}
                        >
          ...
          <p className="s-bd-tz-note">
            Times shown in your time zone. Your tutor teaches from {tutorTimezone}.
          </p>
        </div>

        {!pack.isTrial && (
          <div className="s-bd-total">
            <span className="s-bd-total-label">Total</span>
            <span className="s-bd-total-amt">{total}</span>
          </div>
        )}
        {pack.isTrial && (
          <div className="s-bd-trial-note">
        ...
        {pack.isTrial && (
          <div className="s-field" style={{ marginTop: 16 }}>
            <label className="s-field-label" htmlFor="sintro">
            ...
            <p className="s-bd-intro-hint">
              ...
              {introMessage.trim().length > 0 && introMessage.trim().length < 20 && (
                <span className="s-bd-intro-warn">
        ...
        {withinCutoff && (
          <div className="s-bd-cutoff">
            <p className="s-bd-cutoff-title">Heads up: this lesson is soon.</p>
            <p className="s-bd-cutoff-body">
        ...
        {canRecur && pickedSlot && (
          <label className="s-checkbox-row" style={{ marginTop: 14 }}>
            <input
              type="checkbox"
        ...
        {recurringInfo && <p className="s-form-success" style={{ marginTop: 14 }}>{recurringInfo}</p>}

        {!token && (
          <p className="s-bd-signin-note">
            {pack.isTrial
        ...
        <button
          ...
          className="s-btn s-btn-primary s-btn-lg s-bd-confirm"
        >
          {submitting
            ? 'Loading…'
            : pack.isTrial
...
export default SophiaBookingDialog;
